...
try {
        ...
        const token = jwt.sign({email: userFound.email}, tokenSecret, {expiresIn: '7 days'})
...
const registerUser = async (req, res) => {
    try {
        ...
        const newUser = await userModel.create({ email: req.body.email, password: hashedPassword, secret:tokenSecret});
        res.json(newUser)
    } catch (error) {
